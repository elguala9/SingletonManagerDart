...
''');
      ...
      expect(
        diCode,
        contains(
          "class UserServiceDI extends UserService implements ISingletonStandardDI {",
        ),
      );
      expect(
        diCode,
        contains("db = SingletonDIAccess.get<DatabaseConnection>();"),
      );
    });
...
''');
      ...
      File(
        '${dir.path}/user_service_di.dart',
      ).writeAsStringSync(userServiceCode);
      File(
        '${dir.path}/auth_service_di.dart',
      ).writeAsStringSync(authServiceCode);

      expect(userServiceCode, contains('// AUTO-GENERATED - DO NOT CHANGE'));
      expect(
        userServiceCode,
        contains("class UserServiceDI extends UserService"),
      );
      expect(
        userServiceCode,
        contains("db = SingletonDIAccess.get<DatabaseConnection>();"),
      ...
      expect(
        authServiceCode,
        contains("class AuthServiceDI extends AuthService"),
      );
      expect(
        authServiceCode,
        contains("userService = SingletonDIAccess.get<UserService>();"),
      );
      expect(
        authServiceCode,
        contains("logger = SingletonDIAccess.get<Logger>();"),
      );
    });

    test(
      'should generate correct augmentation with nested directory structure',
      () {
...
''');
      ...
      expect(
        diCode,
        contains(
          "class SimpleServiceDI extends SimpleService implements ISingletonStandardDI {",
        ),
      );
...
''');
    ...
    test(
      'should handle files with multiple classes (some singleton, some not)',
      () {
        final dir = Directory('${tempDir.path}/lib/src');
        dir.createSync(recursive: true);
...
''');
        ...
        expect(
          diCode,
          contains("class SingletonServiceDI extends SingletonService"),
        );
        expect(
          diCode,
          contains("regular = SingletonDIAccess.get<RegularService>();"),
        );
      },
    );
...
''');
      ...
      expect(
        diCode,
        contains("class ConcreteServiceDI extends ConcreteService"),
      );
      expect(
        diCode,
        contains("configuration = SingletonDIAccess.get<String>();"),
      );
    });
...
''');
      ...
      expect(
        diCode,
        contains(
          "class LateFinalServiceDI extends LateFinalService implements ISingletonStandardDI {",
        ),
      );
      expect(
        diCode,
    ...
    test(
      'should generate DI code for class with mixed late and late final fields',
      () {
        final dir = Directory('${tempDir.path}/lib/src');
        dir.createSync(recursive: true);
...
''');
        ...
        // Save generated code to file for inspection
        File(
          '${dir.path}/mixed_field_service_di.dart',
        ...
        expect(
          diCode,
          contains(
            "class MixedFieldServiceDI extends MixedFieldService implements ISingletonStandardDI {",
          ),
        );
        expect(
          diCode,
        ...
        expect(
          diCode,
          contains("config = SingletonDIAccess.get<ConfigManager>();"),
        );
        expect(
          diCode,
          contains("cache = SingletonDIAccess.get<CacheService>();"),
        );
      },
    );
...
''');
      ...
      expect(
        diCode,
        contains(
          "class FinalServiceDI extends FinalService implements ISingletonStandardDI {",
        ),
      );
      expect(
        diCode,
...
''');
        ...
        // Save generated code to file for inspection
        File(
          '${dir.path}/all_modifiers_service_di.dart',
        ).writeAsStringSync(diCode);
        ...
        expect(
          diCode,
          contains(
            "class AllModifiersServiceDI extends AllModifiersService implements ISingletonStandardDI {",
          ),
        );
        expect(
          diCode,
    ...
    group('constructor parameter annotations — parse → generate', () {
      test(
        'mandatory positional param: parser extracts it and generator emits initializeWithParametersDI',
        () {
...
''');
          ...
          expect(
            diCode,
            contains('ApiServiceDI(String baseUrl) : super(baseUrl)'),
          );
          // blank line before factory
          expect(
            diCode,
            contains(
              'ApiServiceDI(String baseUrl) : super(baseUrl);\n\n  factory ApiServiceDI.initializeWithParametersDI',
            ),
          );
          ...
          expect(
            diCode,
            contains(
              'factory ApiServiceDI.initializeWithParametersDI(String baseUrl)',
            ),
          );
          ...
          expect(
            diCode,
            isNot(contains('factory ApiServiceDI.initializeDI()')),
          );
        },
      );
...
''');
        ...
        expect(
          diCode,
          contains(
            'DbServiceDI({required String connectionString}) : super(connectionString: connectionString)',
          ),
        );
        expect(
          diCode,
          contains(
            'factory DbServiceDI.initializeWithParametersDI(String connectionString)',
          ),
        );
        expect(
          diCode,
          contains(
            'final instance = DbServiceDI(connectionString: connectionString)',
          ),
        );
      ...
      test(
        'optional named param: both initializeDI and initializeWithParametersDI are generated',
        () {
          final dir = Directory('${tempDir.path}/lib/src/params');
          dir.createSync(recursive: true);
...
''');
          ...
          expect(
            diCode,
            contains(
              'factory CacheServiceDI.initializeWithParametersDI({int? ttlSeconds})',
            ),
          );
          expect(
            diCode,
            contains(
              'CacheServiceDI({int? ttlSeconds}) : super(ttlSeconds: ttlSeconds)',
            ),
          );
        },
      );
...
''');
        ...
        expect(
          diCode,
          contains(
            'HttpClientDI({required String baseUrl, int? timeoutMs}) : super(baseUrl: baseUrl, timeoutMs: timeoutMs)',
          ),
        );
        ...
        expect(
          diCode,
          contains(
            'factory HttpClientDI.initializeWithParametersDI(String baseUrl, {int? timeoutMs})',
          ),
        );
        expect(
          diCode,
          contains(
            'final instance = HttpClientDI(baseUrl: baseUrl, timeoutMs: timeoutMs)',
          ),
        );
...
''');
        ...
        expect(
          diCode,
          contains(
            'PaymentServiceDI(String apiKey, String? currency) : super(apiKey, currency)',
          ),
        );
        // factory: mandatory=positional, optional=named
        expect(
          diCode,
          contains(
            'factory PaymentServiceDI.initializeWithParametersDI(String apiKey, {String? currency})',
          ),
        );
        expect(
          diCode,
          contains('final instance = PaymentServiceDI(apiKey, currency)'),
        );
        ...
        expect(
          diCode,
          isNot(contains('factory PaymentServiceDI.initializeDI()')),
        );
      });
      ...
      test(
        'repository pattern: @isMandatoryParameter field + @isOptionalParameter ctor param + @isInjected field — initializeWithParametersDI calls initializeDI() then assigns mandatory fields',
        () {
          final dir = Directory('${tempDir.path}/lib/src/params');
          dir.createSync(recursive: true);
          final dartFile = File(
            '${dir.path}/id_handler_storage_repository.dart',
          );
          dartFile.writeAsStringSync('''
import 'package:singleton_manager/singleton_manager.dart';
...
''');
          ...
          expect(
            diCode,
            contains(
              'if (SingletonDIAccess.exists<IOptionalField>()) optionalField = SingletonDIAccess.get<IOptionalField>()',
            ),
          );
          expect(
            diCode,
            contains(
              'if (SingletonDIAccess.exists<IOptionalInjected>()) optionalInjected = SingletonDIAccess.get<IOptionalInjected>()',
            ),
          );
          ...
          expect(
            diCode,
            contains(
              'factory IdHandlerStorageRepositoryDI.initializeWithParametersDI(IWorkDb db, int test, {String? collection, IOptionalField optionalField, IOptionalInjected optionalInjected})',
            ),
          );
          ...
          expect(
            diCode,
            contains('instance.optionalInjected = optionalInjected'),
          );
      ...
      test(
        'repository pattern: @isMandatoryParameter field + @isOptionalParameter ctor param — correct combined output',
        () {
          final dir = Directory('${tempDir.path}/lib/src/params');
          dir.createSync(recursive: true);
...
''');
          ...
          expect(
            diCode,
            contains(
              'factory StorageRepoDI.initializeWithParametersDI(IWorkDb db, {String? collection})',
            ),
          );
          expect(diCode, contains('instance.db = db'));
...
''');
        ...
        expect(
          diCode,
          contains(
            'SmtpServiceDI(String host, int port, bool? useTls) : super(host, port, useTls)',
          ),
        );
        expect(
          diCode,
          contains(
            'factory SmtpServiceDI.initializeWithParametersDI(String host, int port, {bool? useTls})',
          ),
        );
        expect(
          diCode,
          contains('final instance = SmtpServiceDI(host, port, useTls)'),
        );
...
''');
        ...
        expect(parsed, hasLength(1));
        expect(
          parsed[0].injectedFields[0].fieldType,
      ...
      test(
        'nullable generic: IRepository<BookData>? is preserved in exists<> and get<>',
        () {
...
''');
          ...
          final diCode = AugmentationGenerator.generate(parsed[0]);
          File(
            '${dir.path}/nullable_service_di.dart',
          ).writeAsStringSync(diCode);

          expect(
            diCode,
            contains(
              'if (SingletonDIAccess.exists<IRepository<BookData>>()) repo = SingletonDIAccess.get<IRepository<BookData>>();',
            ),
          );
        },
      );
...
''');
        ...
        expect(
          diCode,
          contains(
            'cache = SingletonDIAccess.get<ICache<Map<String, int>>>();',
          ),
        );
      });
...
''');
        ...
        expect(
          diCode,
          contains('map = SingletonDIAccess.get<IMap<String, BookData>>();'),
        );
      });
    });
  });
  ...
  group('Integration Tests - generic artifact files', () {
    const paramsDir = 'test_artifacts/integration_tests/lib/src/params';
    ...
    test(
      'ErmesBookServiceBase with IErmesBookRepository<BookData>: parse → generate → write artifact',
      () {
